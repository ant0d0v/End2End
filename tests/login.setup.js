...
setup('Login to swisscows site', async ({ page }) => {
  ...
  await page.getByPlaceholder('Username or email').fill('a.qa@swisscows.email');
  await page.getByPlaceholder("Password").fill("Qwerty!1");
  await page.getByRole("button", { name: "Login" }).click();
  await page.getByRole("img", { name: "Swisscows", exact: true });
