...
export class HeaderStaticPages extends BasePage {
    constructor(page) {
        ...
        //Locators
        this.linksOfHeader = (name) => this.page.getByRole('link', { name: name })
    ...
    clickHamburgerMenuButton = async () => {
        await this.clickElement(this.hamburgerMenu, `hamburger menu in the header static pages`);
    }
    clickSearchCounter= async () => {
        await this.clickElement(this.searchCounter, `charity search counter  in the header`);
    }
    ...
    AssertTextCharitySearchCounterToHave = async (text) => {
        await this.AssertTextOfElement(this.charitySearchCounter, text)
    };
    AssertPopupCharitySearchCounterToHaveText = async (text) => {
        await this.AssertTextOfElement(this.popupCharitySearchCounter, text)
    }
